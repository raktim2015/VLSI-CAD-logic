...
Idea : We can represent a boolean expression using Shanon cofactor: F(x,y,z) = x.F(x==1) + x'.F(x==0) If we take the complement F(x,y,z)' = (x.F(x==1) + x'.F(x==0))' .... F(x,y,z)' = x.(F(x==1))' + x'.(F(x==0))'
