Course materials and assignments for VLSI-CAD Logic course on Coursera
https://www.coursera.org/learn/vlsi-cad-logic

Topics covered:

Week-1
	Introduction

Week-2
	1) 	Shanon's expansion
	2) 	Cofactor. Single and multi-variable cofactoring.
	3) 	Properties of cofactor
	4) 	Derivate of a boolean function. (Boolean difference)
	5) 	Universal and existential quantification
	6) 	Network repair
	7) 	Positional Cube Notation
	8) 	Recursive tautology checking
	9) 	Implementation of Unate Recursive Paradigm (URP)
	10) Complement of a function using URP

Week-3
	a)	BDD
		1)	Binary Decision Diagram (BDD) and Reduced Ordered Binary Decision Diagram (ROBDD)
		2) 	Reduction rules from BDD to ROBDD
		3) 	Building BDD incremenatally. Divide and Conquer
		4)	Applications including tautology checking and SAT.
	b)	SAT
		1) CNF form.
		2) Boolean Constraints Propagation for SAT solver
		3) Application of SAT in network repairs and others

Week-4
	a)	2 Level Logic Minimization
		1) Best vs Good enough solutions
		2) Reduce-Expand-Irredundant Optimization Loop
	b) Multi-level logic and Boolean Network Model
		1) Simplify a node, remove a node and adding new node(factoring)
		2) Algebraic division for boolean expressions.
		3) Kernels and Co-Kernels in factoring
 
